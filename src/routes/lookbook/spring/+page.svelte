<script>
  ...
  const springSuitCategories = [
    {
      name: 'Lightweight Wool',
      description: 'Experience the comfort of lightweight wool suits, perfect for spring occasions.',
    ...
    {
      name: 'Linen Blends',
      description: 'Stay cool and stylish with our linen blend suits, ideal for warmer days.',
      image: 'https://images.unsplash.com/photo-1594938298603-c8148c4dae35?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1160&q=80'
    },
    {
      name: 'Pastel Tailoring',
      description: 'Embrace the season with our pastel-colored suits that add a fresh touch to your wardrobe.',
      image: 'https://images.unsplash.com/photo-1594938298603-c8148c4dae35?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1160&q=80'
    },
    {
      name: 'Bespoke Fit',
      description: 'Our bespoke suits are tailored to your exact measurements for the perfect fit.',
      image: 'https://images.unsplash.com/photo-1594938298603-c8148c4dae35?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1160&q=80'
    }
  ];
</script>
...
<div class="flex manrope flex-col bg-[#0C0404] items-center min-h-screen w-screen text-white">
  <div class="relative w-full h-[50vh] bg-cover bg-black bg-center flex items-center justify-center">
    ...
    <h1 class="relative top-40 text-4xl md:text-6xl lg:text-7xl uppercase tracking-widest text-center marcellus">Spring Bespoke Suits</h1>
  </div>
  
  <div class="max-w-7xl mx-auto px-4 py-16">
    {#if visible}
      <p in:fade={{duration: 1000, delay: 300}} class="text-lg marcellus text-center mb-12 max-w-3xl mx-auto">
        Discover our exquisite spring bespoke suits, where craftsmanship meets seasonal style.
      </p>
    {/if}

    <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
      {#each springSuitCategories as category, index}
        {#if visible}
          <div in:fly={{y: 50, duration: 1000, delay: 300 * (index + 1)}} class="group relative overflow-hidden">
            ...
            <div class="absolute inset-0 bg-black bg-opacity-40 transition-opacity duration-300 group-hover:bg-opacity-60 flex flex-col justify-end p-6">
              ...
              <!-- <a class="text-white font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-300 hover:text-amber-400" href={`/lookbook/spring/${category.name.toLowerCase().replace(/\s+/g, '-')}-23`} >
                View Collection</a> -->
            </div>
          </div>
        {/if}
    ...
    {#if visible}
      <div in:fade={{duration: 1000, delay: 1500}} class="mt-16 text-center">
        <h2 class="text-3xl mb-6 marcellus">Elevate Your Spring Wardrobe</h2>
        <p class="mb-8">Experience the art of bespoke tailoring with our spring collection.</p>
